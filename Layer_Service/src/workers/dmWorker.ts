import miar from "../../../Layer_Library/src/miar"
import MiarError from "../../../Layer_Library/src/miar-error";
 ...
 */
    private async createAndPublishVideoAsync(
        keyType: KeyType,
        accessToken: string,
        ...
        const res = await miar.miarAxios.axiosAsync({
            url: urlByKeyType,
            method: "POST",
            headers: {
                Authorization: "Bearer " + accessToken,
                "Content-Type": miar..urlencoded,
            },
            data: {
    ...
    private async stopConsumingUntilSpecificTimeAsync(
        ...
        miar.miarLog.log("Info", logMsg)
    }
    private async getAcccessTokenByPasswordAsync(
        apiKey: string,
        apiSecret: string,
        ...
        try {
            ...
            const axiosRes = await miar.miarAxios.axiosAsync({
                url: shared.getBaseUrl("public") + "/oauth/token",
                method: "POST",
                headers: {
                    "Content-Type": miar.miarType.URL_ENCODED
                },
                data: {
            ...
            if (!axiosRes.success) throw miar.miarError
                .setStatus((axiosRes.data as MiarErrorModel).status)
                .setMessage((axiosRes.data as MiarErrorModel).message);

            // when any error occured in dailymotion api (THROW)
            else if ("error" in axiosRes.data) throw miar.miarError
                .setStatus(500)
                .setMessage(axiosRes.data.error_description);

            return axiosRes.data as AccessTokenResForPassword;
        } catch (err: any) {
            miar.miarLog.log("Error", "AccessToken - Password", {
    ...
    private async getAcccessTokenByClientCredentialsAsync(
        apiKey: string,
        apiSecret: string,
        scopes: PermissionScope[]
    ) {
        try {
            ...
            miar.miarLog.log("Error", "AccessToken - Client", {
                error_message: err.message
            });
            return undefined;
        }
    ...
    private async isAccessTokenValidAsync(qName: string, grantType: GrantType) {
     ...
     */
    private async updateAccessTokenIfRequired(
        grantType: "password",
        qName: string,
        apiKey: string,
        apiSecret: string,
    ...
    private getBaseUrl(keyType: KeyType) {
    ...
    private async publishVideoAsync(
        keyType: KeyType,
        accessToken: string,
        videoId: string,
        title: string,
     ...
     */
    private async createVideoAsync(
        keyType: "public",
        accessToken: string,
        downloadLink: string,
        userId: string
    ...
    public async uploadVideoByPasswordAsync() {
        try {
            ...
            await shared.miarMq.channel?.consume(qName, async (msg) => {
                // when queue is empty
                if (!msg) {
                    miar.miarLog.log("Info", "Queue is empty.")
                    return;
                };
                ...
                { // consume queue
                    ...
                    try {
                        ...
                        if (!isSuccess) throw miar.miarError.setStatus(5000);
                        ...
                        if (!creationAndPublishRes.success) throw miar.miarError
                            .setStatus(5001)
                            .setMessage((creationAndPublishRes.data as MiarErrorModel).message);
                        ...
                        miar.miarLog.log("Info", "Video is uploaded to dailymotion.", {
                            username,
                            video_id: data.id,
                        ...
                        await this.stopConsumingUntilSpecificTimeAsync(
                        ...
                        miar.miarLog.log("Error", "Video didn't uploaded to dailymotion.", {
                        ...
                        switch (miarErr.status) {
                            case 5000:  // Access Token Error
                                await this.stopConsumingUntilSpecificTimeAsync(
                                    2,
                                    this.uploadVideoByPasswordAsync
                                );
                                break;
            ...
            const miarErr: MiarError = err;
            miar.miarLog.log("Error", `General - Worker of "uploadVideo" didn't executed. ` +
                `(error_message: ${miarErr.message})`);
        }
    }

    public async uploadVideoByClientCredentialsAsync() {
        try {
            ...
            await shared.miarMq.channel?.consume(qName, async (msg) => {
                ...
                { // consume queue
                    ...
                    try {
                        ...
                        // save log
                        const miarErr: MiarError = err;
                        miar.miarLog.log("Error", "Consume - Video didn't uploaded to dailymotion.", {
                            error_message: miarErr.message,
                            body
                        });
